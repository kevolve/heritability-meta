...
#'
#'
#' @usage mlm.variance.distribution(x, suppress.plot=F)
#'
...
#' @param suppress.plot A logical for whether to hide the plot or include it in output. Defaults to TRUE.
#'
...
mlm.variance.distribution = function(x, suppress.plot=F){
	...
	g = ggplot(df, aes(fill=Level, y=Variance, x=as.factor(Type))) +
		...
		# Level 1
		annotate("text", x = 2, y = (df[1,2]/2+df[2,2])/100, label = paste("Level 1 (sampling error): \n",
																		   round(df$Variance[3],2), "%", sep=""), size = 3) +
		
		# Level 2
		annotate("text", x = 2, y = (df[5,2]+(df[4,2]/2))/100,
				 label = bquote(italic(I)[Level2]^2*" (estimate ID):"~.(round(df[4,2],2))*"%"), size = 3) +
		
		# Level 3
		annotate("text", x = 2, y = (df[5,2]/2)/100,
				 label = bquote(italic(I)[Level3]^2*" (study ID):"~.(round(df[5,2],2))*"%"), size = 3)
	
	# print(df.res)
	# cat("Total I2: ", round(totalI2, 2), "% \n", sep="")
	if(suppress.plot==F) suppressWarnings(print(g))
	# invisible(df.res)
	df.res
}
